...
원칙: 기술 토큰은 원문 유지, 모든 설명·서술·결정사항은 ${langName}.`;
     ...
     */
    buildContextBootstrapRules(agentRole) {
...
이 신호가 세션 로그에 없으면 해당 세션의 산출물은 "맥락 미검증" 으로 간주해 재작업 대상이 된다.`;
    ...
    /** 에이전트 전용 스킬 섹션 생성 */
    buildSkillsSection(agent, providerType) {
        const lines = ['\n\n## 사용 가능한 스킬'];
...
- metrics.json에 구조화된 데이터 저장 (자동 관리)`;
     ...
     */
    buildPhaseRules(phase, component) {
        if (!phase)
            return null;
...
3. 통합 중 발견된 실패는 작은 패치로 해결한다 (큰 재설계는 새 태스크로).`;
    ...
    buildMeetingAndConsultationGuide() {
        return `\n\n## 다중 참여자 회의 & 자문 전문가
