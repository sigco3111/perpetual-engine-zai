...
import { ConsultantFactory } from '../agent/consultant-factory.js';
...
export class Orchestrator {
    ...
    messageQueue;
    meetingCoordinator;
    consultantFactory = new ConsultantFactory();
    /** 활성 자문 에이전트 추적 (ID → ConsultantAgent) */
    activeConsultants = new Map();
    watcher = null;
    messageWatcher = null;
     ...
     */
    processingRoles = new Map();
    workflowAborters = new Map();
    ...
    async start() {
        ...
        try {
            ...
            logger.error(`고아 태스크 재개 오류: ${err.message}`);
        }
        ...
        try {
            ...
            logger.error(`초기 태스크 스캔 오류: ${err.message}`);
        }
    ...
    async stop() {
        ...
        for (const aborter of this.workflowAborters.values())
            aborter.abort();
     ...
     */
    async drainProcessingTasks(maxWaitMs = 1500) {
    ...
    async maybeStartCeo() {
        ...
        logger.info(`기존 태스크 ${tasks.length}개, 스프린트 ${sprints.length}개 감지 — CEO 자동 기동 건너뜀 ` +
    ...
    startWatcher() {
        ...
        this.watcher.on('change', async () => {
            ...
            try {
                ...
                logger.error(`태스크 처리 오류: ${err.message}`);
            }
        });
    }
    async processNewTasks() {
        ...
        const pendingTasks = tasks.filter(t => (t.status === 'backlog' || t.status === 'todo') && !this.processingTasks.has(t.id));
     ...
     */
    async resumeInFlightTasks() {
        const tasks = await this.kanban.getAllTasks();
        const resumeStatuses = new Set(['in_progress', 'testing', 'review']);
        const candidates = tasks.filter(t => resumeStatuses.has(t.status)
        ...
        for (const task of candidates) {
            ...
            this.dispatchWorkflow(task, `고아 태스크 재개: ${task.id} - ${task.title} (phase: ${task.phase ?? 'planning'})`);
            busyRoles.add(lockRole);
        }
    }
    /**
     ...
     */
    dispatchWorkflow(task, announceMessage) {
        const lockRole = task.assignee;
        ...
        this.workflowEngine.runWorkflow(task, aborter.signal).then(release).catch(err => {
            logger.error(`[${task.id}] 워크플로우 실패: ${err.message}`);
            release();
        });
    }
    ...
    startMessageWatcher() {
        ...
        this.messageWatcher.on('add', async (filePath) => {
            if (!this.running || !filePath.endsWith('.json'))
                return;
            try {
                await this.processNewMessages();
            }
            catch (err) {
                logger.error(`메시지 처리 오류: ${err.message}`);
            }
        });
    }
    ...
    async dispatchMessageToAgent(agent, messageContent, targetRole, replyTo) {
        try {
            const allTasks = await this.kanban.getAllTasks();
            const builder = new PromptBuilder();
            const kanbanSummary = builder.buildKanbanSummary(allTasks);
            ...
            logger.error(`메시지 에이전트 시작 실패: ${err.message}`);
            await this.messageQueue.send({
                from: targetRole,
                to: replyTo,
                type: 'info',
                content: `[${targetRole}] 에이전트 시작 실패: ${err.message}`,
            });
            return;
        }
        this.watchAgentReply(targetRole, replyTo);
    }
    watchAgentReply(role, replyTo) {
        const pollMs = 5000;
        let elapsed = 0;
     ...
     */
    async handleConsultationRequest(msg) {
        try {
            const request = typeof msg.content === 'string'
            ...
            logger.error(`자문 요청 파싱 실패: ${err.message}`);
        }
    }
    /**
    ...
    async handleMeetingInvite(msg) {
        try {
            ...
            logger.error(`회의 초대 파싱 실패: ${err.message}`);
        }
    }
    // ========== 태스크 강제 제어 ==========
    /**
    ...
    async forceRunTask(taskId) {
        ...
        this.workflowEngine.runWorkflow(task, aborter.signal).then(release).catch(err => {
            logger.error(`[${taskId}] 워크플로우 실패: ${err.message}`);
            release();
        });
    }
    /**
     ...
     */
    async suspendTask(taskId, reason) {
     ...
     */
    watchConsultantCompletion(consultantId) {
     ...
     */
    async disposeConsultant(consultantId) {
    ...
    async startMultiAgentMeeting(params) {
        ...
        if (!initiator)
            throw new Error(`주최자 에이전트를 찾을 수 없습니다: ${initiatorRole}`);
        // 참여 에이전트들 조회
        const participants = [];
     ...
     */
    watchMeetingCompletion(meetingId, consultants) {
