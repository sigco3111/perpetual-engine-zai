import { componentExpectedOutputs, manifestPath, techStackDocPath, } from './components.js';
/** 미설정 페이즈의 기본 타임아웃 — 10분 */
export const DEFAULT_PHASE_TIMEOUT_MS = 10 * 60 * 1000;
function planningDocPath(slug) {
    return `docs/planning/feature-${slug}.md`;
}
function designDocPath(slug) {
    return `docs/design/feature-${slug}.md`;
}
function developmentDocPath(slug) {
 ...
 */
function buildStaticPhases(taskSlug) {
    return [
        {
            name: 'planning',
 ...
 */
function buildComponentPhases(taskSlug, manifest) {
    const componentPhases = manifest.components.map((spec, idx, arr) => {
...
function buildPostDevelopmentPhases(taskSlug) {
    return [
        {
            name: 'testing',
            taskStatus: 'testing',
 ...
 */
export function buildPhases(taskSlug, manifest) {
 ...
 */
export function resolvePhaseAlias(phase) {
    if (!phase)
        return null;
 ...
 */
export function findPhase(phases, name, instanceKey) {
    for (let i = 0; i < phases.length; i++) {
        const p = phases[i];
        if (p.name !== name)
            continue;
        if (instanceKey && p.instanceKey !== instanceKey)
            continue;
 ...
 */
export function phaseInstanceKey(phase) {
