import { buildPhases, findPhase, phaseInstanceKey, resolvePhaseAlias, DEFAULT_PHASE_TIMEOUT_MS, } from './phases.js';
...
export class WorkflowEngine {
    ...
    static MAX_PHASE_RETRIES = 2;
    async runWorkflow(task, signal) {
        const taskSlug = String(task.id).toLowerCase().replace(/[^a-z0-9]/g, '-');
        const startPhase = resolvePhaseAlias(task.phase) ?? 'planning';
        ...
        try {
            while (cursor < phases.length) {
                ...
                logger.step(`[${task.id}] ${phase.name}${labelSuffix} 페이즈 시작 (담당: ${phase.leadAgent})` +
                ...
                if (success) {
                    ...
                    if (phase.name === 'development-plan' && !manifest) {
                        ...
                        if (fresh) {
                            ...
                            logger.info(`[${task.id}] 컴포넌트 매니페스트 로드 — ${manifest.components.length}개 컴포넌트로 페이즈 펼침`);
                            continue;
                        }
            ...
            logger.error(`[${task.id}] 워크플로우 예외 발생: ${err.message}`);
        }
    ...
    async runMetricsCheckIfNeeded() {
        try {
            ...
            logger.error(`메트릭스 평가 오류: ${err.message}`);
        }
    }
    /** 메트릭스 평가 지시 메시지 생성 */
    buildMetricsEvalInstruction(taskId, metrics, evalType) {
...
4. 달성률에 따라 다음 행동(scale_up/maintain/iterate/pivot/kill)을 결정하고 실행하세요
   ...
   - kill: 관련 태스크를 done으로 마감하고 사유 문서화`;
    ...
    async executePhase(task, phase, signal) {
    ...
    async waitForCompletion(role, maxWait = 600000, signal) {
