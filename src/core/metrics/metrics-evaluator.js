...
export class MetricsEvaluator {
     ...
     */
    calculateAchievement(metric, actual) {
    ...
    calculateOverallAchievement(results) {
        if (results.length === 0)
            return 0;
    ...
    /** 달성률로부터 판정 결정 */
    getVerdict(overallAchievement) {
        if (overallAchievement >= 120)
            return 'exceeded';
        if (overallAchievement >= 100)
            return 'achieved';
        if (overallAchievement >= 60)
            return 'improving';
        if (overallAchievement >= 30)
            return 'stagnant';
        return 'failed';
    }
    /** 판정으로부터 다음 행동 결정 */
    getAction(verdict, type) {
    ...
    /** 판정별 한국어 설명 생성 */
    generateReasoning(verdict, action, results, plan, type) {
        const lines = [];
        ...
        for (const r of results) {
            ...
            lines.push(`- ${r.name}: ${r.actual}${matchingDef?.unit ?? ''} ` +
    ...
    evaluate(params) {
        ...
        // 각 지표 결과 계산
        const results = actuals.map(({ name, actual }) => {
