...
export class MetricsManager {
    ...
    async getTaskMetrics(taskId) {
        ...
        if (!entry)
            return null;
        if (!isTaskMetrics(entry))
            return null;
        return entry;
    }
    /** 태스크에 메트릭스 계획 등록 */
    async setPlan(taskId, plan) {
    ...
    /** 평가 결과 추가 */
    async addEvaluation(taskId, evaluation) {
        await this.store.update((data) => {
            if (!data.tasks[taskId]) {
    ...
    async getTasksNeedingEvaluation() {
        const data = await this.getAll();
        ...
        for (const [taskId, metrics] of Object.entries(tasks)) {
            ...
            if (!isTaskMetrics(metrics))
                continue;
            const { plan, evaluations } = metrics;
            // 이미 최종 평가가 끝났으면 스킵
            if (evaluations.some(e => e?.type === 'final'))
                continue;
            ...
            // 체크포인트 도래 확인
            const evaluatedCheckpoints = new Set(evaluations
                .filter(e => e?.type === 'checkpoint' && typeof e.evaluated_at === 'string')
                .map(e => e.evaluated_at.slice(0, 10)));
            const dueCheckpoints = plan.checkpoints.filter(cp => typeof cp === 'string' && cp <= now && !evaluatedCheckpoints.has(cp.slice(0, 10)));
