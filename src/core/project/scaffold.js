...
export async function scaffoldProject(projectRoot, projectName, options) {
    ...
    if (options?.scannedMeta) {
        ...
        if (meta.name)
            config.product.name = meta.name;
        if (meta.mission)
            config.company.mission = meta.mission;
    ...
    config.agents = agentConfigs.map(a => a.role === 'custom' ? toSafeFileName(a.name) : a.role);
    ...
    // 빈 kanban.json 초기화
    await writeFileIfNotExists(paths.kanban, JSON.stringify({ tasks: [], next_id: 1 }, null, 2));
    // 빈 sprints.json 초기화
    await writeFileIfNotExists(paths.sprints, JSON.stringify({ sprints: [], current_sprint: null }, null, 2));
    // 빈 metrics.json 초기화
    await writeFileIfNotExists(paths.metrics, JSON.stringify({ tasks: {} }, null, 2));
