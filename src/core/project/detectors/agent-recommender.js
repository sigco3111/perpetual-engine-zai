 ...
 */
export function recommendAgents(techStack, docs) {
    const reasoning = [];
    const agents = [];
...
// --- 필요성 판단 ---
function needsDesigner(ts, docs) {
    const hasUiFramework = ts.frameworks.some(f => ['React', 'Vue.js', 'Svelte', 'Angular', 'Next.js', 'Nuxt.js',
...
function buildCto(ts, docs, reasoning) {
...
function buildPo(ts, docs, reasoning) {
    const methodologies = [];
    if (docs.workflowKeywords.includes('scrum'))
        methodologies.push('스크럼');
    if (docs.workflowKeywords.includes('kanban'))
        methodologies.push('칸반');
    if (docs.workflowKeywords.includes('agile'))
        methodologies.push('애자일');
...
function buildDesigner(ts, docs, reasoning) {
...
function buildQa(ts, docs, reasoning) {
...
function buildMarketer(ts, docs, reasoning) {
    const channels = [];
    if (ts.platforms.includes('Web'))
        channels.push('SEO', '콘텐츠 마케팅');
    if (ts.platforms.includes('iOS'))
        channels.push('App Store');
    if (ts.platforms.includes('Android'))
...
/** 도메인 특화 자문역 생성 */
function buildDomainAdvisors(docs, reasoning) {
    const advisors = [];
    const domainAdvisorMap = {
        'fitness': {
...
//# sourceMappingURL=agent-recommender.js.map