...
import { detectDocs } from './detectors/docs-detector.js';
 ...
 */
export async function scanExistingProject(projectRoot) {
...
// ─── 모드 A: CLAUDE.md 기반 ───────────────────────────────────
async function buildFromClaudeMd(projectRoot, claudeMd, scannedAgents, techStack, docs) {
    const agentConfigs = [];
    const reasoning = [];
...
// ─── 모드 B: 자동 감지 기반 ───────────────────────────────────
function buildFromAutoDetect(projectRoot, techStack, docs) {
...
// ─── 기술 스택 요약 포맷 ─────────────────────────────────────
function formatTechSummary(ts) {
    const parts = [];
    if (ts.languages.length > 0)
        parts.push(ts.languages.join(', '));
    if (ts.frameworks.length > 0)
        parts.push(ts.frameworks.join(', '));
    if (ts.platforms.length > 0)
        parts.push(`[${ts.platforms.join('/')}]`);
    if (ts.isMonorepo)
        parts.push('(모노레포)');
    return parts.join(' | ');
}
...
function parseAgentBlock(englishName, koreanDesc, block, isAdvisor) {
...
// ─── 에이전트 설정 빌드 ──────────────────────────────────────
async function loadRulesFile(projectRoot, scanned) {
...
function extractRulesFromContent(content, maxRules = 10) {
    const rules = [];
...
function extractResponsibilities(scope, rulesContent) {
...
function buildAgentConfig(scanned, rulesContent, techStack) {
...
// ─── 추론 헬퍼 ───────────────────────────────────────────────
function inferCollaborators(role) {
    const map = {
        ceo: ['cto', 'po', 'marketer'],
        cto: ['po', 'designer', 'qa'],
        po: ['ceo', 'cto', 'designer'],
...
function inferRequiredMeetings(role) {
    const map = {
...
// ─── 시스템 프롬프트 생성 ────────────────────────────────────
function buildSystemPromptFromScan(scanned, rulesContent, techStack) {
    const lines = [];
...
// ─── 프로젝트 메타데이터 추출 ────────────────────────────────
function parseProjectMeta(claudeMd, techStack, docs) {
