...
export class SessionManager {
     ...
     */
    async createTmuxSession(sessionName, fullCmd) {
    ...
    async startAgent(params) {
        ...
        try {
            ...
            this.configureConcurrency(config);
        }
        catch {
        ...
        if (configProvider) {
            ...
            const createComplete = new Promise((resolve) => this.createCompleteResolvers.set(agent.role, resolve));
            const execute = async () => {
                try {
                    ...
                    await new Promise((resolve) => this.slotResolvers.set(agent.role, resolve));
                }
                finally {
    ...
    async isAgentRunning(role) {
        return this.tmux.hasSession(role);
    }
    async getAgentLog(role, lines = 100) {
        const logDir = this.getLogDir();
        if (!logDir)
            return '';
     ...
     */
    toAdapterConfig(configProvider) {
        return {
     ...
     */
    buildMcpConfigObject(agent, config) {
     ...
     */
    buildMcpConfigArg(agent, config) {
        const requiredTools = agent.required_mcp_tools;
        if (!requiredTools?.length)
            return null;
        const mcpServers = config.mcp_servers;
    ...
    async startMeetingSession(params) {
        ...
        const participantContext = participants.map(p => `### ${p.name} (${p.role})\n- 역할: ${p.description}\n- 책임: ${p.responsibilities.join(', ')}`).join('\n\n');
        const consultantContext = consultants?.map(c => `### ${c.config.name}\n- 역할: ${c.config.description}\n- ID: ${c.id}`).join('\n\n') ?? '';
        const meetingPrompt = `\n\n## 회의 정보
- 회의 ID: ${meetingId}
