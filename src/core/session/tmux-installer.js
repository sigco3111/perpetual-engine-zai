...
async function detectLinuxPkgMgr() {
    const candidates = [
 ...
 * - 그 외 플랫폼: 수동 설치 안내
 */
export async function tryAutoInstallTmux(platform = osPlatform()) {
    if (platform === 'darwin') {
        if (await commandExists('brew')) {
            try {
                ...
                return {
                    ...
                    succeeded: false,
                    method: 'brew',
                    message: `brew install tmux 실패: ${err.message}`,
                };
            }
        }
        return {
            attempted: false,
            succeeded: false,
            message: 'Homebrew 가 설치되어 있지 않습니다. https://brew.sh 에서 설치 후 재시도하거나 "brew install tmux" 를 수동 실행하세요.',
        };
    }
    if (platform === 'linux') {
...
//# sourceMappingURL=tmux-installer.js.map