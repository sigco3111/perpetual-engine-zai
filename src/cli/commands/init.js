            ...
            if (scanResult) {
                ...
                if (ts.languages.length > 0 || ts.frameworks.length > 0) {
                    ...
                    if (ts.languages.length > 0)
                        stackParts.push(`언어: ${ts.languages.join(', ')}`);
                    if (ts.frameworks.length > 0)
                        stackParts.push(`프레임워크: ${ts.frameworks.join(', ')}`);
                    if (ts.platforms.length > 0)
                        stackParts.push(`플랫폼: ${ts.platforms.join(', ')}`);
                    if (ts.backend.length > 0)
                        stackParts.push(`백엔드: ${ts.backend.join(', ')}`);
                    if (ts.databases.length > 0)
                        stackParts.push(`DB: ${ts.databases.join(', ')}`);
                    if (ts.isMonorepo)
                        stackParts.push('모노레포');
