...
export function registerSetupCommand(program) {
    program
        ...
        const choice = providerAnswers.providerChoice ?? (providerAnswers.useZaiProvider ? 'zai-api' : 'claude-code');
        ...
        await writeFile(path.join(paths.vision, 'company-goal.md'), `# ${answers.companyName} - 회사 목표\n\n## 미션\n${answers.companyMission}\n\n## 핵심 가치\n${answers.coreValue}\n\n---\n*이 문서는 모든 에이전트의 의사결정 기준이 됩니다.*\n`, 'utf-8');
        await writeFile(path.join(paths.vision, 'product-vision.md'), `# ${answers.productName} - 프로덕트 비전\n\n## 개요\n${answers.productDescription}\n\n## 타겟 사용자\n${answers.targetUsers}\n\n## 핵심 가치\n${answers.coreValue}\n\n## 기술 스택\n- 선호: ${answers.techStackPreference}\n- 배포: ${answers.deployTarget}\n\n---\n*이 문서는 PO와 CTO의 기획/개발 기준이 됩니다.*\n`, 'utf-8');
