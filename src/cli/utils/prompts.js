...
export async function runProviderSetupPrompts() {
    const providerChoice = await select({
        message: 'AI 프로바이더를 선택하세요:',
        choices: [
            { name: 'Claude Code CLI (원본)', value: 'claude-code' },
            { name: 'OpenCode CLI (oh-my-openagent)', value: 'opencode' },
            { name: 'ZAI GLM API (HTTP API 전용)', value: 'zai-api' },
            { name: 'OpenCode + ZAI 혼합 (추천: 코딩은 OpenCode, 나머지는 ZAI)', value: 'mixed-opencode-zai' },
            { name: 'Claude Code + ZAI 혼합 (코딩은 Claude, 나머지는 ZAI)', value: 'mixed-claude-zai' },
        ],
        default: 'mixed-opencode-zai',
    });
