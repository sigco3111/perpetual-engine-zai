...
import { loadConfig, saveConfig } from '../core/project/config.js';
...
export class DashboardServer {
    ...
    onRestartAgents;
    onTaskResumed;
    constructor(projectRoot, port = 3000, options) {
    ...
    setupRoutes() {
            ...
            try {
                ...
                if (this.onTaskResumed) {
                    this.onTaskResumed(req.params.id).catch(err => {
                        logger.error(`태스크 재개 디스패치 오류: ${err.message}`);
                    });
                }
        ...
        this.app.get('/api/docs/:category/:filename', async (req, res) => {
            try {
                const dir = paths[req.params.category];
                if (!dir) {
        ...
        this.app.post('/api/agents/pause-all', async (_req, res) => {
            try {
                ...
                for (const role of this.agentRegistry.getRoles()) {
                    try {
                        await this.sessionManager.pauseAgent(role);
                    }
                    catch { /* skip */ }
                }
                res.json({ success: true });
